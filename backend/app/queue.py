...
def enqueue_provider_job(task_id: str, delay_seconds: int = 0, at_front: bool = False) -> None:
    queue = task_queue()
    ...
    if delay_seconds > 0:
        ...
        return
    queue.enqueue("app.worker.process_provider_job", task_id, at_front=at_front, **kwargs)
...
def enqueue_result_finalize_job(task_id: str, provider_job_id: str, result: dict, delay_seconds: int = 0) -> None:
    queue = result_queue()
    kwargs = {
        "job_timeout": settings.result_finalize_job_timeout_seconds,
        "result_ttl": 3600,
        "failure_ttl": 86400,
