...
def _output_key(task_id: str, params: dict[str, Any] | None = None) -> str:
    ...
    if run_suffix:
        return f"{task_id}-{run_suffix}-enhanced.mp4"
    return f"{task_id}-enhanced.mp4"
...
def _run_enhance_command(
    ...
    if not settings.enhance_command:
        raise VideoProcessingError("enhance command is not configured.")

    work_dir = output_path.with_suffix(".enhance-work")
    ...
    command = [
        ...
        for part in shlex.split(settings.enhance_command)
    ]
    try:
