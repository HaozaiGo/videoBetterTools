...
平台 worker 默认 `WORKER_REPLICAS=6`，结果入库 worker 默认 `RESULT_WORKER_REPLICAS=4`，ZIP 预打包 worker 默认 `ZIP_WORKER_REPLICAS=8`。GPU 服务当前按 `MODEL_PLAZA_GPU_DEVICE_IDS=4,5,6,7` 和 `MODEL_PLAZA_GPU_WORKERS_PER_DEVICE=2` 分配，也就是 GPU4/GPU5/GPU6/GPU7 各最多同时处理 2 个模型任务，平台侧先最多同时提交 6 个远端视频任务，给 GPU API 的 8 个服务端 slot 留 2 个缓冲。GPU 服务设置 `MODEL_PLAZA_GPU_UPLOAD_RESULTS=0` 后只生成结果，平台 worker 通过 GPU API 拉取 `output.mp4` 后投递 `model-plaza-results`，由结果 worker 上传对象存储并入库，避免上传链路占住 GPU 入口 worker。ProPainter 并发仍需持续观察显存。
