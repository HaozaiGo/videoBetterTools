...
平台 worker 默认 `WORKER_REPLICAS=8`，结果入库/远端轮询 worker 默认 `RESULT_WORKER_REPLICAS=8`，ZIP 预打包 worker 默认 `ZIP_WORKER_REPLICAS=8`。GPU 服务当前按 `MODEL_PLAZA_GPU_DEVICE_IDS=4,5,6,7` 和 `MODEL_PLAZA_GPU_WORKERS_PER_DEVICE=2` 分配，也就是 GPU4/GPU5/GPU6/GPU7 各最多同时处理 2 个模型任务。平台入口 worker 只提交远端 GPU job，然后把轮询、GPU 结果确认、结果入库交给 `model-plaza-results`；GPU 服务收到平台生成的 TOS 预签名上传地址后直接上传结果，避免“平台拉取远端结果”和“TOS 上传”占住入口 worker。ProPainter 并发仍需持续观察显存。
