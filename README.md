...
平台 worker 默认 `WORKER_REPLICAS=8`，ZIP 预打包 worker 默认 `ZIP_WORKER_REPLICAS=8`。GPU 服务当前按 `MODEL_PLAZA_GPU_DEVICE_IDS=3,4,5,6` 和 `MODEL_PLAZA_GPU_WORKERS_PER_DEVICE=2` 分配，也就是 GPU3/GPU4/GPU5/GPU6 各最多同时处理 2 个模型任务，平台侧最多同时提交 8 个远端视频任务和 8 个 ZIP 打包任务。GPU 服务设置 `MODEL_PLAZA_GPU_UPLOAD_RESULTS=0` 后只生成结果，平台 worker 通过 GPU API 拉取 `output.mp4` 再上传对象存储，避免 GPU 侧被结果上传链路占住。ProPainter 并发仍需持续观察显存。
